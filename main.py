...
hunians.append(Rumah("Gustavo Fring", 5, 2, 1300))
hunians.append(Indekos("Chuck McGill", "Howard Hamlin", 450))
hunians.append(Rumah("Mike Ehrmantraut", 1, 4, 1300))
root = Tk()
...
def details(index):
    ...
    if(hunians[index].get_jenis() != "Indekos"):
        ...
        iterator += 1
    d_dokumen = Label(d_frame, text="Dokumen: " + hunians[index].get_dokumen(), anchor="w").grid(row=2+iterator, column=0, sticky="w")
...
for index, h in enumerate(hunians):
    ...
    if h.get_jenis() != "Indekos": 
        name = Label(frame, text=" " + h.get_nama_pemilik(), width=60, borderwidth=1, relief="solid", anchor="w")
        name.grid(row=index, column=2)
    else:
        name = Label(frame, text=" Penghuni " + h.get_nama_penghuni() + " Pemilik " + h.get_nama_pemilik(), width=60, borderwidth=1, relief="solid", anchor="w")
        name.grid(row=index, column=2)
